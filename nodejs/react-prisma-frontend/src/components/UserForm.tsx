...
export const UserForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedUser = {
        ...user,
        data_aniversario: new Date(user.data_aniversario), // Converte a data para um objeto Date
      };
    ...
    <div>
      <form onSubmit={handleSubmit} className="mb-4">
        ...
        <input
          ...
          onChange={(e) => setUser({ ...user, data_aniversario: e.target.value })}
          className="block mb-2 p-2 border"
        />
        ...
        <tbody>
          {users.map((user) => (
            <tr key={user.id} className="hover:bg-gray-100">
              ...
              <td className="border border-gray-300 p-2">{new Date(user.data_aniversario).toLocaleDateString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
