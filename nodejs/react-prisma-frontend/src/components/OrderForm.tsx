...
export const OrderForm = () => {
  const [order, setOrder] = useState({
    ...
    status: '',
    data: '',
    itens: [{ id_produto: 0, qtde: 1, preco: 0 }],
  });
  ...
  const addItemToOrder = () => {
    setOrder({
      ...order,
      itens: [...order.itens, { id_produto: 0, qtde: 1, preco: 0 }],
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const formattedOrder = {
            ...order,
            data: new Date(order.data), // Converte a data para um objeto Date
          };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="mb-4">
        {/* Seleção de Usuário */}
        <select
          value={order.id_usuario}
          onChange={(e) => setOrder({ ...order, id_usuario: parseInt(e.target.value) })}
          ...
          {orders.map((order) => (
            <tr key={order.id} className="hover:bg-gray-100">
              ...
              <td className="border border-gray-300 p-2">
                {users.find((user) => user.id === order.id_usuario)?.nome}
              </td>
                  ...
                  <tbody>
                    {order.itens.map((item) => (
                      <tr key={item.id_produto}>
                        <td className="border border-gray-300 p-2">
                          {products.find((product) => product.id === item.id_produto)?.nome}
                        </td>
